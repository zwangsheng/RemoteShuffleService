...
object ControlMessages extends Logging {
  ...
  object RegisterShuffleClient {
    def apply(topology: String, bindAddress: String): PbRegisterShuffleClient =
  ...
  object UnRegisterShuffleClient {
    def apply(topology: String, bindAddress: String): PbUnRegisterShuffleClient = {
      PbUnRegisterShuffleClient.newBuilder().setTopology(topology)
        .setBindAddress(bindAddress).build()
    }
  }
  ...
  case class GetPartitionLocationResponse(
      status: StatusCode,
      hosts: Seq[String]) extends MasterMessage
